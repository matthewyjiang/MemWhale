...
fn run() -> Result<i32, String> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            value if value.starts_with("--") => {
                return Err(format!("unknown option {value:?}; run mw-run --help"));
            }
            value => {
    ...
    if command_parts.is_empty() {
        return Err("missing command; run mw-run --help".to_string());
    }
    ...
    eprintln!("mw-run: remembered command run #{run_id}");
