...
fn record_session(notes: String) -> Result<(), String> {
    ...
    let transcript_path = sessions_dir.join(format!("session-{}.log", started_at.replace(':', "-")));
    ...
    let status = Command::new("script")
        ...
        .status()
        .map_err(|err| {
            format!("failed to launch `script` (is it installed?): {err}")
        })?;
...
fn print_help() {
    println!(
        "mw [--notes <text>]      record a whole shell session until you exit\n\
         ...
         mw global on|off|status  auto-record every new terminal (no .bashrc editing)\n\
         \n\
...
fn shell_rc_path() -> Result<PathBuf, String> {
    ...
    let is_zsh = shell.rsplit('/').next().map_or(false, |name| name.contains("zsh"));
    Ok(home.join(if is_zsh { ".zshrc" } else { ".bashrc" }))
}

fn global_hook_path() -> Result<PathBuf, String> {
    Ok(data_base()?.join("MemoryWhale").join("global-hook.sh"))
}

fn global_enabled_path() -> Result<PathBuf, String> {
    Ok(data_base()?.join("MemoryWhale").join("global-enabled"))
}
...
fn show_session(args: &[String]) -> Result<(), String> {
    ...
    let conn = Connection::open(database_path()?).map_err(|err| format!("failed to open db: {err}"))?;
    init_schema(&conn)?;
    ...
    match row {
        ...
        Err(rusqlite::Error::QueryReturnedNoRows) => {
            Err(format!("no session #{id}; run `mw list` to see recorded sessions"))
        }
...
fn list_sessions() -> Result<(), String> {
    ...
    let mut count = 0;
    for row in rows {
        let (id, started_at, byte_count, notes) =
...
fn sessions_dir() -> Result<PathBuf, String> {
    Ok(data_base()?.join("MemoryWhale").join("sessions"))
}
